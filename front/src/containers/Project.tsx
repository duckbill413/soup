  ...
  return (
    <div className={styles.container}>
      <button onClick={()=>router.push(`/project/${projectId}/outline`)} type="button">
        <img className={styles.projectImage} src={projectImageUri} alt="프로젝트 이미지" width={PROJECT_IMAGE_WIDTH} height={PROJECT_IMAGE_HIGHT} />
        <p className={styles.projectName}>{projectName}</p>
      </button>
      <div>
        {projectMemberImageUris?.map((member: ProjectMemberImageUri) =>
          member.projectMemberImageUri &&
          <img src={member.projectMemberImageUri} alt="프로젝트 멤버 이미지" width={MEMBER_IMAGE} height={MEMBER_IMAGE} />)
        }
      </div>
    </div>
  )
